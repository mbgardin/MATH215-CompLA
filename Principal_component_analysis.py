...
mean_neg = np.mean(X_neg_2D, axis=1)
mean_pos = np.mean(X_pos_2D, axis=1)

dist_alice_neg = np.linalg.norm(Alice_2D - mean_neg)
dist_alice_pos = np.linalg.norm(Alice_2D - mean_pos)

dist_bob_neg = np.linalg.norm(Bob_2D - mean_neg)
dist_bob_pos = np.linalg.norm(Bob_2D - mean_pos)

Alice_prediction = 1 if dist_alice_pos < dist_alice_neg else -1

Bob_prediction = 1 if dist_bob_pos < dist_bob_neg else -1
